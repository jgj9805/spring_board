...
<div class="container lmember">
	<h1 class="mt-3 mb-5 text-center">회원목록 (관리자전용)</h1>
	<div class="text-end">
...
%>
	<tr>
		...
		<td><%=userid %></td>
		<td><%=username %></td>
		<td>[<%=postcode %>] <%=addr %>d <%=detailAddr %></td>
		<td><%=tel %></td>
		<td><%=wdate %></td>
		<td> 
			<%
		...
		%>
		<select name="level" id="level" onChange="memLevel(this, <%=level%>, <%=num%>);">
		...
		<ul class="pagination justify-content-center">
			<%
				...
				if(startNum > 1){
					...
					out.print("<li class=\"page-item\"><a class=\"page-link\" href=\"?fname=member/memberlist&page="+prevPage+"\">Previous</a></li>");
				}else{
			...
			<%
				...
				if(endNum < totalPage){
					...
					out.print("<li class=\"page-item\"><a class=\"page-link\" href=\"?fname=member/memberlist&page="+nextPage+"\">Next</a></li>");
				}else{
